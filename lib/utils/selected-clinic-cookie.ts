...
export async function getClinicServer(c?: any): Promise<SelectedClinic | null> {
...
export function getClinicClient(): SelectedClinic | null {
  ...
  if (!raw) return null;
  try { return JSON.parse(decodeURIComponent(raw)); } catch { Cookies.remove(NAME); return null; }
}
...
export function setClinicClient(value: SelectedClinic | null) {
  ...
  if (!value) { Cookies.remove(NAME); return; }
